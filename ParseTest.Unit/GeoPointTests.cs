  ...
  public class GeoPointTests {
    ...
    public void TestGeoPointCultureInvariantParsing() {
      ...
      foreach (var c in CultureInfo.GetCultures(CultureTypes.AllCultures)) {
        ...
        var serialized = AVClient.SerializeJsonString(
          new Dictionary<string, object> {
            { "point", NoObjectsEncoder.Instance.Encode(point) }
          });
        var deserialized = AVDecoder.Instance.Decode(
          AVClient.DeserializeJsonString(serialized)) as IDictionary<string, object>;
